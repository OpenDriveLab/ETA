...
class Plotter(object):
    ...
    def dot(self, pos, node, color=(255, 255, 255), label=None, r=2):
        x, y = 5.5 * (pos - node)
        x += self.size / 2
        y += self.size / 2
        ...
        # If a label is provided, write it
        if label:
            self.draw.text((x, y), label, fill=color)
...
class RoutePlanner(object):
    def __init__(
        self, min_distance, max_distance, debug_size=256, lat_ref=42.0, lon_ref=2.0
    ):
        self.route = deque()
    ...
    def run_step(self, gps, return_both=False):
        self.debug.clear()
        ...
        for i in range(1, len(self.route)):
            ...
            self.debug.dot(
                ...
                self.route[i][0],
                (r, g, b),
                label=f"{distance:.2f}_{to_pop}_{farthest_in_range:.2f}",
            )
        ...
        if return_both:
            return (
                (self.route[1], self.route[2])
                if len(self.route) > 2
                else (self.route[1], self.route[1])
            )
