...
def get_weather_id(weather_conditions, args, config):
    from xml.etree import ElementTree as ET

    tree = ET.parse(os.path.join(args.b2d_path, "leaderboard/data/weather.xml"))
    root = tree.getroot()
...
class LeaderboardEvaluator(object):
    ...
    def _load_and_run_scenario(self, args, config):
        ...
        weather_id = get_weather_id(config.weather[0][1], args, config)
        currentDateAndTime = datetime.now()
        currentTime = currentDateAndTime.strftime("%m_%d_%H_%M_%S")
        save_name = (
            f"{route_name}_{town_name}_{scenario_name}_{weather_id}_{currentTime}"
        )
    ...
    def run(self, args):
        ...
        while route_indexer.peek() and not crashed:
            ...
            if crashed:
                print(
                    f"{route_indexer.index} crash, [{route_indexer.index}/{route_indexer.total}], please restart",
                    flush=True,
                )
                break
