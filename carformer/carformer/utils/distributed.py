...
def ddp_setup(args, dist_url="env://", use_deepspeed=False):
    ...
    if "RANK" in os.environ and "WORLD_SIZE" in os.environ:
        ...
        raise ValueError("Rank and world size must be set. Please make sure you are using torchrun")
    
        # Single gpu: 
        # python train_hydra_ds.py +deepspeed=zero3
        # 8 gpus, with 10 cpus per GPU: 
        # OMP_NUM_THREADS=10 torchrun --nproc_per_node=8 train_hydra_ds.py +deepspeed=zero3
    ...
    if use_deepspeed:
        ...
        ds.init_distributed(dist_backend="nccl")
    else:
        init_process_group(
